...
class CacheManager {
  ...
  async deletePartyFromCache(partyId) {
    const db = await this.init();
    const tx = db.transaction("parties", "readwrite");
    await wrapIDBRequest(tx.objectStore("parties").delete(partyId));
    return wrapIDBRequest(tx);
  }
  ...
  async clearAllCache() {
    ...
    const storeNames = [
      ...
      "users",
      "inventory_macros",
      "party_materials_details",
      "cache_metadata",
    ];

    for (const storeName of storeNames) {
      const tx = db.transaction(storeName, "readwrite");
      await wrapIDBRequest(tx.objectStore(storeName).clear());
    }
  }
}
