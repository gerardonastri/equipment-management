...
export function PartyFormModal({
  ...
  const responsabiliIds = Array.isArray(party.responsabili_ids)
    ? party.responsabili_ids
    : [];
  const selectedResponsabili = responsabiliIds
    .map((id) => users.find((u) => u.id === id))
    .filter(Boolean);
  const availableResponsabili = responsabiliList.filter(
    (u) => !responsabiliIds.includes(u.id),
  );

  const addResponsabile = (id) => {
    if (id && !responsabiliIds.includes(id))
      onPartyChange({ ...party, responsabili_ids: [...responsabiliIds, id] });
  };
  const removeResponsabile = (id) => {
    onPartyChange({
      ...party,
      responsabili_ids: responsabiliIds.filter((rid) => rid !== id),
    });
  };

  const animatoriIds = Array.isArray(party.animatori_ids)
    ? party.animatori_ids
    : [];
  const selectedAnimatori = animatoriIds
    .map((id) => users.find((u) => u.id === id))
    .filter(Boolean);
  const availableAnimatori = animatoriList.filter(
    (u) => !animatoriIds.includes(u.id),
  );

  const addAnimatore = (id) => {
    if (id && !animatoriIds.includes(id))
      onPartyChange({ ...party, animatori_ids: [...animatoriIds, id] });
  };
  const removeAnimatore = (id) => {
    onPartyChange({
      ...party,
      animatori_ids: animatoriIds.filter((aid) => aid !== id),
    });
  };

  const driversIds = Array.isArray(party.drivers_ids) ? party.drivers_ids : [];
  const selectedDrivers = driversIds
    .map((id) => users.find((u) => u.id === id))
    .filter(Boolean);
  const availableDrivers = driversList.filter(
    (u) => !driversIds.includes(u.id),
  );

  const addDriver = (id) => {
    if (id && !driversIds.includes(id))
      onPartyChange({ ...party, drivers_ids: [...driversIds, id] });
  };
  const removeDriver = (id) => {
    onPartyChange({
      ...party,
      drivers_ids: driversIds.filter((did) => did !== id),
    });
  };
  ...
  const handoffableMacros = macroCategories.filter((m) =>
  ...
  const handleSubmitWithAlert = (e) => {
    ...
    if (missingResponsabile || missingMagazziniere) {
      ...
      setTimeout(() => {
        document
          .getElementById("staff-alert-banner")
        ...
        <form onSubmit={handleSubmitWithAlert} className="space-y-4">
          ...
          <div className="p-3 bg-indigo-50/50 border border-indigo-100 rounded-lg">
            <label className="block text-sm font-medium text-indigo-900 mb-2">
              <span className="flex items-center gap-1.5">
                <UserCheck className="w-4 h-4 text-indigo-600" /> Responsabili
                evento
                {missingResponsabile && (
            ...
            {selectedResponsabili.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-2">
                {selectedResponsabili.map((u) => (
                  <span
                    key={u.id}
                    className="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-sm font-medium"
                  >
                    {u.nome}
                    <button
                      type="button"
                      onClick={() => removeResponsabile(u.id)}
            ...
            <div className="flex gap-2">
              <select
                ...
                onChange={(e) => {
                  if (e.target.value) {
                    addResponsabile(e.target.value);
                    e.target.value = "";
                  }
                }}
              ...
              >
                <option value="">
                  {availableResponsabili.length === 0
                    ...
                    : responsabiliIds.length === 0
                      ? "Aggiungi responsabile..."
                      : "Aggiungi un altro responsabile..."}
                </option>
            ...
            >
              <option value="">
                {availableAnimatori.length === 0
                  ...
                  : animatoriIds.length === 0
                    ? "Aggiungi animatore..."
                    : "Aggiungi un altro animatore..."}
              </option>
                ...
                >
                  {isSyncingAnimatori ? (
                    <>
                      ...
                      Sincronizzando dal gestionale...
                    </>
                  ) : (
                    <>
                      <RefreshCw className="w-3.5 h-3.5" />
                      Sincronizza animatori dal gestionale
                    </>
                  )}
                </button>
                <AnimatePresence>
                        ...
                        <div className="text-xs space-y-0.5">
                          {syncInfo.added > 0 ? (
                            ...
                            <p className="text-muted-foreground">
                              Nessun nuovo animatore da aggiungere
                            </p>
                          )}
                          {syncInfo.unmatched?.length > 0 && (
                            <p className="text-amber-600">
                              ⚠ Non trovati nel DB:{" "}
                              {syncInfo.unmatched.join(", ")}
                            </p>
                          )}
                        </div>
                      )}
            ...
            {selectedDrivers.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-2">
                {selectedDrivers.map((u) => (
                  <span
                    key={u.id}
                    className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-200 text-slate-700 rounded-full text-sm font-medium"
                  >
                    {u.nome}
                    <button
                      type="button"
                      onClick={() => removeDriver(u.id)}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <ul className="text-xs text-amber-700 mt-1 space-y-0.5">
                        ...
                        {missingMagazziniere && (
                          <li>• Nessun magazziniere assegnato</li>
                        )}
                      </ul>
          ...
          <div>
            ...
            {currentUserRole === "amministratore" ? (
              ...
              <div className="w-full px-3 py-2 border border-input rounded-lg bg-surface/50 text-muted-foreground cursor-not-allowed">
                {party.stato === "iniziale"
                  ? "Iniziale"
                  : party.stato === "caricato_scaffale"
                    ? "Caricato sullo Scaffale"
                    : party.stato === "caricato_furgone"
                      ? "Caricato nel Furgone"
                      : party.stato === "scaricato_furgone"
          ...
          <div>
            ...
            <div className="relative mb-2.5">
              ...
              <input
                type="text"
                placeholder="Cerca macro-categoria (es. Consolle, Piastre...)"
                value={materialSearch}
                onChange={(e) => setMaterialSearch(e.target.value)}
                className="w-full pl-9 pr-3 py-2 border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-ring text-sm bg-surface"
              />
            </div>

            <div
              className={`border border-border rounded-xl p-4 max-h-48 overflow-y-auto mb-3 relative transition-opacity ${isLoadingMacros ? "opacity-50 pointer-events-none" : ""}`}
            >
                ...
                <div className="space-y-1.5">
                  {filteredMacros.map((macro) => {
                    ...
                    const isUsedElsewhere =
                      usedMacroIds?.has(macro.id) && !isSelected;
                    return (
                      <label
                        key={macro.id}
                      ...
                      >
                        <input
                          ...
                          checked={isSelected}
                          onChange={() =>
                            !isUsedElsewhere && onMaterialToggle(macro.id)
                          }
                  ...
                  <div className="border border-amber-200 rounded-xl bg-amber-50/50 p-4 mt-2 mb-4">
                    ...
                    {(selectedSingleItems?.length || 0) > 0 && (
                      <p className="text-xs text-amber-700 font-medium mt-3 px-1 border-t border-amber-200/50 pt-2">
                        ...
                        selezionat{selectedSingleItems.length === 1 ? "o" : "i"}
                      </p>
                    )}
                  </div>
                </motion.div>
          ...
          <div className="border border-border rounded-xl overflow-hidden">
            ...
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors ${handoffEnabled || party._handoffOpen ? "bg-violet-500 text-white" : "bg-muted text-muted-foreground"}`}
              >
                <ArrowRightLeft className="w-4 h-4" />
              </div>
              <div className="flex-1 text-left">
                <p
                  className={`text-sm font-semibold ${handoffEnabled || party._handoffOpen ? "text-violet-800" : "text-foreground"}`}
                >
                  Passaggio Materiale a un'altra Festa
                </p>
                <p className="text-xs text-muted-foreground">
                  Il magazziniere salta il controllo finale/iniziale — il
                  responsabile gestisce il passaggio.
                </p>
              </div>
              <div
                className={`w-11 h-6 rounded-full transition-colors shrink-0 relative ${handoffEnabled || party._handoffOpen ? "bg-violet-500" : "bg-gray-200"}`}
              >
                <div
                  className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${handoffEnabled || party._handoffOpen ? "translate-x-6" : "translate-x-1"}`}
                />
              </div>
            </button>
                      ...
                      {handoffCandidates.length === 0 ? (
                        <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                          ...
                          <p className="text-xs text-amber-700">
                            Nessuna altra festa attiva trovata per questa data.
                            Crea prima l'altra festa, poi torna qui.
                          </p>
                        </div>
                      ) : (
                        <select
                          ...
                          onChange={(e) =>
                            onPartyChange({
                              ...party,
                              handoff_to_party_id: e.target.value || null,
                              _handoffOpen: true,
                            })
                          }
                    ...
                    {handoffableMacros.length > 0 ? (
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mb-2">
                          Il materiale NON selezionato torna normalmente al
                          magazzino con il check standard.
                        </p>
                        <div className="space-y-1.5 max-h-40 overflow-y-auto">
                          {handoffableMacros.map((macro) => (
                            <label
                              key={macro.id}
                              className="flex items-center gap-3 p-2.5 rounded-lg bg-white border border-violet-100 cursor-pointer hover:border-violet-300 transition-colors"
                            >
                              <input
                                type="checkbox"
                      ...
                      <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                        <Info className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                        <p className="text-xs text-amber-700">
                          Assegna prima il materiale alla festa per scegliere
                          cosa passa alla festa successiva.
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="p-4 border border-violet-100 bg-violet-50/30 rounded-xl">
              <label className="block text-sm font-medium text-violet-900 mb-3 flex items-center gap-2">
                <ArrowRightLeft className="w-4 h-4" /> Scaffali Virtuali
                (Passaggio Materiale)
              </label>
                ...
                {Array.from({ length: 12 }, (_, i) => `V${i + 1}`).map((vs) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isLocked}
                      onClick={() =>
                        isSelected ? onRemoveShelf(vs) : onAddShelf(vs)
                      }
                      className={`w-14 h-12 flex flex-col rounded-lg items-center justify-center text-sm font-bold transition-all border relative ${
                        isSelected
                      ...
                      title={
                        isLocked
                          ? "Scaffale già assegnato a 2 feste per handoff"
                          : ""
                      }
                    >
                      <span>{vs}</span>
                      {usageCount === 1 && !isSelected && (
                        <span className="text-[9px] font-normal leading-none opacity-80 mt-0.5">
                          In attesa
                        </span>
                      )}
                    </button>
              ...
              <p className="text-xs text-violet-600/80 mt-3">
                Usa questi scaffali (es. V1) per permettere il check di
                materiale in transito tra due feste. Assegna lo stesso scaffale
                virtuale ad entrambe le feste coinvolte nell'handoff.
              </p>
            </div>
          </div>
